...
- 诊断第一版的 AI 痕迹：口号式短句、自问自答、拟人（"自己教你怎么用""更新会主动找你"）、
  句尾补一句安抚、满屏"你"、"……就行 / 都行 / 也一样"句式反复。
- 中文改用本土修辞，平实句与修辞句穿插：喜鹊衔物的比喻（magpie 的本义）、成语俗语
  （顺藤摸瓜、一网打尽、有迹可循、按图索骥、如法炮制、查漏补缺、不在话下、一式多份）、
  自然对偶（"中文问，英文答""字面和意思，两头都顾"）。不用自问自答，不用句尾安抚。
- 英文不套修辞，改为平实直述，去掉口号、拟人、设问。
- 左侧导航与标题长度：新标题一度折行（中文卡片 4/18，英文章节 2/7、卡片 6/18），逐个缩短到
  一行内；对照改文案前（中文章节 2/7 折行、英文章节 4/7 卡片 9/18 折行）。

## 连续三轮干净

- R1（机制）：导航章节与卡片标题折行 中 0/7、0/18，英 0/7、0/18；页面 1280 / 390 宽中英四种
  组合无截断、无横向滚动。
- R2（静态）：docs-parity 0 FAIL；页面文案键中英一致；18 张卡片中英齐全；rest 32/32；长破折号 0；
  问号 0。
- R3（可复现）：导航与布局检查连跑两次，8 行输出逐行一致，折行 0，横向滚动 0。

---
# 验收记录 2026-09-26（网站文案改为通俗亲和的说法；手机布局修复）

用户要求网站文案"用人类的话说，通俗易懂，亲和的语气"。

## 改动

- `docs/content.js` 中英全部重写：事实不变，去掉术语（RRF、FTS5、SigLIP、ONNX、PP-OCR、
  嵌入、loopback 等），必要处用一句白话解释；补上多浏览器合并显示图标、设置分页、Web 里的
  浏览器提示。`docs-parity` 要求的关键词全部保留。`index.html` 的英文默认文字与 meta 描述
  同步。
- 手机布局（改动前就存在，量化后修复）：
  - "更多功能""隐私"网格最小列宽 400/380px 大于手机宽度，右侧文字被切，iOS 可横向拖动 →
    `minmax(min(400px, 100%), 1fr)`。
  - 首屏 900px 光晕把页面撑到 645px（390px 视口）→ `width: min(900px, 100vw)`；先试过
    `overflow-x: clip` 裁剪，截图显示在 `.hero` 边距处出现硬边，弃用。
  - 顶栏按钮在手机上折成竖排（"功\n能"）→ 不换行；520px 以下隐藏"功能"锚点、收窄边距。

## 连续三轮干净

- R1（静态）：docs-parity 0 FAIL（rest 32/32）；页面文案键中英一致（30）；18 张卡片都有
  中英两版；长破折号 0；术语残留仅键名 `stat.embed`（不显示）。
- R2（机制，Chromium 1280 与 390 宽、中英）：`documentElement.scrollWidth` 390 → 390（改前
  645）；无元素越过视口右缘（改前"更多功能"条目 right=420）；顶栏按钮均为单行（高 32–38px）；
  四种组合均无横向滚动、无截断。截图目测桌面与手机首屏。
- R3（可复现）：同一布局检查连跑两次，输出逐行一致，横向滚动 0。

---
# 验收记录 2026-09-26（#7 选中跳行、#5 提示与键位间距、#6 设置分区，0.4.4）

## 实现

- #7：行（及动作菜单）的悬停只在指针屏幕坐标真的变化时改选中；window 捕获阶段记录
  坐标，窗口获得焦点时清空。分批显示时，若 React 尚未渲染本次上一批列表，不沿用旧选中。
- #5：Web 标签输入非空时，在文字后显示淡色「⌘⏎ 在浏览器中搜索」（canvas 按输入框字体
  量宽定位，放不下则隐藏）；`Keys` 组件把修饰键与主键分开（3px），Windows/Linux 用
  `Ctrl`/`Alt`。底栏按优先级让位：状态放不下时从最不需要的提示开始隐藏（操作、设置
  永不隐藏），字体加载完重测。
- #6：设置分 通用 / 本地文件 / Web / GitHub Stars / 剪贴板 / 关于，打开时落在当前标签页
  对应页；标题与分页固定、内容滚动。新增 Settings › Web「已发现的浏览器」（`web_sources`
  命令：每个浏览器的书签/历史数，图标，立即同步）。两条小贴士的设置路径更新。

## 发现并修复

| # | 阶段 | 问题 | 修复 |
|---|------|------|------|
| 1 | 复现（T23 修复前 3/6） | 结果长到静止指针下时，原地 mousemove 把那一行选中（选中第 8 行）；方向键选的行也被原地事件抢走 | 只认真实移动 |
| 2 | review | 分批显示竞态：上一批未渲染时用旧查询的选中项找行 | 以"上次交给 React 的列表是否已上屏"判断 |
| 3 | review | 动作菜单悬停同类问题 | 同样只认真实移动 |
| 4 | 开发 | 提示计算放在 `source` 等声明之前（TDZ） | 移到声明之后 |
| 5 | 基线测量（既有） | 英文底栏在改动前就截断状态：720px 下 Mac 160/172、Windows 124/172 | 按优先级让位 |
| 6 | 截图目测 | sticky 分页导航背景 0.97 不透明，内容透出 | 改为标题与导航固定、仅内容区滚动 |
| 7 | 静态 | 两条小贴士指向旧分组名（外观与行为、系统） | 改为 通用、关于 |
| 8 | review（误判） | 以为无依赖的测量 effect 会多一次布局，加了依赖 → 字体晚到时不再重测，Windows 英文又截断 154/172 | 恢复每次渲染测量（绘制前本就要布局），并在 fonts.ready 后强制重测 |
| 9 | 契约 | T9 断言底栏含小写 `ctrl⏎` | 改为不区分大小写（有意改为 Ctrl） |

搬迁核对：重组前后设置区域按去缩进行做多重集比较，旧 1355 行仅少 5 条分组注释与不再使用的
"索引"小标题，其余 32 项内容一行未丢。

## 连续三轮干净（最终构建）

- R1（机制 + 回归，真机隔离配置）：T20 14/14、T13 5/5、T15 18/18、T9 30/30、T22 11/11、
  T23 6/6、T26 6/6（真实 chrome 335/2029、edge 344/4；从 Web 标签打开落在 Web 页；六页均
  有设置且从顶部开始；从本地文件标签打开落在本地文件页）；demo：T24 12/12（mac/win ×
  zh/en，720px 状态均完整，长查询隐藏提示，非 Web 无提示）、T25（滚动 900px 导航不动）、
  T20b 3/3。
- R2（静态 + review）：clippy 0、cargo test 202、tsc 0、a1/a3/docs-parity 0 FAIL（i18n 322
  键全引用）、搬迁核对如上；最后改动复查无发现。
- R3（可复现）：cargo test ×2 各 204 行一致；T23 ×2、T26 ×2 各 6/6 逐行一致；T24 ×2 各
  12/12 输出逐字节一致。

---
# 验收记录 2026-09-25（Web：多浏览器合并显示图标、语义结果降级，未出包）

issue #5 追加：同一书签在多个浏览器都有时显示各浏览器小图标（建议 50% 灰度）；截图里
搜 bili / test 都返回同样 3 个不相关书签。

## 语义下限校准（`core/examples/verify_web_floor.rs`，真实 e5-small）

- 20 个书签、20 个"应按意思找到"的查询（29 对）、18 个无关查询：
  应找到的相似度中位 0.844（最低 0.787），无关查询的最高命中中位 0.830、最高 0.854。
- 减去查询自身基线（对 30 条背景页面的平均相似度）后：应找到中位 0.038，噪声中位
  0.035。两种阈值都无法区分；语义 top-1 正确率 13/20。
- 用户选方案 2：保留但降级。

## 实现

- `search_bookmarks` / `search_history`：命中带 `browsers`（该 URL 所在全部浏览器，
  排序）与 `fuzzy`（该 URL 没有任何副本被关键词/子串命中）。新建 `bookmarks(url)`、
  `history(url)` 索引。
- `rank_web`：存在任何关键词命中则去掉全部 fuzzy；否则最多 `WEB_FUZZY_MAX`=3 条。
  面板与 MCP 共用。
- `apps::browser_app`：浏览器名找已安装应用（精确 / "X Browser" > 以名结尾 > 以名开头，
  同级取最短）；`browser_icon` 命令复用应用图标缓存。
- 前端：行尾 `BrowserIcons`（14px、`grayscale(50%)`，无应用时首字母），"可能相关"虚线
  徽章，预览列出全部浏览器；README 双语改写 Web 段；demo `?fuzzy=1`。

## 发现并修复

| # | 阶段 | 问题 | 修复 |
|---|------|------|------|
| 1 | 校准 | 计划的"语义下限"不成立（见上） | 改为有关键词命中时去掉语义结果，否则最多 3 条并标注 |
| 2 | 截图目测 | 浏览器图标全显示成首字母：StrictMode 先挂载、卸载再挂载，首次请求回来时组件已卸载，第二次挂载看缓存已占位就跳过 | 共享进行中的 Promise，每个实例都等它 |
| 3 | 仪器 | T22 180 s 内等不到语义结果：全新配置在补算向量，`try_lock` 降级 | 等待放宽到 600 s（实测约 4 分钟） |

## 连续三轮干净

- R1（机制，真机 release、隔离配置、本机真实 Chrome/Edge 数据只读）：T22 11/11：
  "github" 40 行，每 URL 一行，9 行同时属于 chrome+edge；图标取自已装应用（PNG），
  未装的浏览器为空；无意义查询 3 条全部 fuzzy；界面 40 行 47 个 IMG 图标
  `grayscale(0.5)`，关键词结果无"可能相关"，无意义查询 3 行都带。demo 截图目测布局。
- R2（review + 迁移 + 回归）：diff 复查无发现；真实旧库副本（679 书签 / 1316 历史）打开
  后两索引建立，查询计划 `SEARCH bookmarks USING INDEX bookmarks_url`；T20 14/14、
  T13 5/5、T15 18/18、T9 30/30。
- R3（可复现）：cargo test ×2 各 204 行逐行一致、0 失败；T22 ×2 各 11/11 逐行一致
  （合并行都是 9）；a1 / a3-ipc / docs-parity 0 FAIL。
...
同时等应用、系统命令、文件三路，应用被文件搜索拖住；文件搜索要把查询送进 e5（图像
模型已加载时还要送进 SigLIP 文本端），机器忙时一秒以上。无 Mac 实测，根因为推断，
所以同时加了分段计时日志，用户的日志能证实或推翻。

## 实现

- 前端：三路各自 `.then` 到达即画（`paint`），只合并已到的部分；首次画选第一行，
  后到部分按 `hitKey` 保住用户已移动的选择；`seq` + `sourceRef` 票据丢弃过期回答；
  全部到齐前若无任何结果则保留旧列表不闪空。确认态/动作菜单按选中行身份
  （`selectedKey`）失效，列表只是变长时不被打断。
- 图标：未读到时显示名字首字母（`.app-monogram`，与图标同尺寸），读到后替换。
- 后台刷新：搜索出应用后 `refresh_app_icons`（fire-and-forget）逐个比对
  `icon_stamps`（Info.plist / 快捷方式 mtime），变了才重读并发 `app-icons-changed`；
  变更在锁内先占位，连续按键不会重复读同一图标。
- `search_apps`：应用列表锁在打开数据库前释放；>50 ms 记 `slow app search`。
- `search_local`：>150 ms 记 `slow file search: 总 ms（text model / image model /
  waiting / ranking）`，不记查询内容。测试钩子 `MAGPIE_TEST_SLOW_LOCAL_MS` 模拟慢文件搜索。
- demo `?noicons=1`：app_icon 返回空，用于检查首字母占位。

## 发现并修复（计数清零轮）

| # | 视角 | 问题 | 修复 |
|---|------|------|------|
| 1 | 并发 | 计时用全局 `static EMBED_MS`，连续按键的并发搜索互相覆盖计时 | 改为闭包返回值，局部变量 |
| 2 | 并发 | 连续按键触发多个 `refresh_app_icons`，同一变化图标可能被重复读 | 锁内比对并先写入新 stamp 占位 |
| 3 | 代码正确 | 字符串 `Replace` 把 `embedded_at` 插进了另外 4 个同构函数（clippy 抓到） | 只保留 `search_local` 一处 |
| 4 | 静态一致 | `AppIcon` 文档注释仍写"空方框占位" | 改为首字母 |
| 5 | 仪器 | T20 判定用"应用"，中文实际是"应用程序"；T13/T15/T9 runner 找不到 mjs | 修正判定；运行前拷入 magpie-promo |

## 连续三轮干净

- R1（机制通路 + 回归，最终构建、隔离档案）：T20 14/14（文件搜索压到 1.5 s 时应用行
  160 ms 出现、文件行 1666 ms 接到末尾；1.5 s 内移动的选择在文件到后保持；缓存图标随行
  出现；未变图标不重读；改快捷方式 mtime 后日志 `app icons: 1 refreshed after a search`
  且前端收到事件，应用行仍 164 ms 出现；慢搜索日志含分段）；T20b 3/3（demo 无图标时
  "V" 26×26 占位，有图标时 IMG 26×26）；回归 T13 5/5、T15 18/18、T9 30/30；cargo test
  192 通过；clippy 0。
- R2（可复现）：cargo test ×2 各 193 行（含 ignored）逐行一致、0 失败；T20 ×2 各 14/14
  逐行一致（毫秒数归一后）；a1 / a3-ipc / docs-parity 0 FAIL。
- R3（边界与退化）：T20c 8/8：120 ms 内连打两个查询，旧查询的慢回答从未覆盖新列表；
  无任何匹配时到齐后列表为空无残留；只有文件无应用时文件行出现并选中首行；web 搜索照常。

## 未验证

- Mac 真机：根因（嵌入耗时）为推断；0.4.2 的 `slow file search` 日志行用于证实。
...
JSON 输出，结构化表格需 tagged PDF，大多数没有）。

## 实现

- `files::pdf_to_markdown(path, ocr)`：`extract_pages_markdown` 逐页；有文字层的页用
  pdf-inspector 的 Markdown；否则看页上有没有图像：有图就交给 OCR（引擎开着时），
  没开标注"第 N 页是扫描件"；无图但有乱码文字标注"读不准"；空白页跳过。全部读不出
  时报错并提示开 OCR。不用索引里存的内容（可能被截断）。
- 命令 `pdf_markdown` / `save_pdf_markdown`：只接受已索引文件夹里的 `.pdf`；另存只写
  `.md` / `.markdown`（路径来自原生保存对话框，默认 PDF 旁同名）。
- 前端：PDF 行的 Ctrl/Cmd+K 菜单多"复制为 Markdown""另存为 Markdown…"；底栏短时
  提示（转换中 / 已复制 N 字 / 已保存）；启动小贴士一条；README 双语、网站一条。

## 发现并修复

| # | 阶段 | 问题 | 修复 |
|---|------|------|------|
| 1 | 开发单测 | pdf-inspector 的逐页 `needs_ocr` 对空白页也置位、对纯图片页反而没置位：空白页被当成扫描件、扫描页被当空白丢掉 | 改为看页上是否有图像判断扫描件 |

## 连续三轮干净（覆盖整个 0.4.0）

- 静态：clippy 0；a1 72/72、a3 96 处调用 + 301 个 i18n 键、docs-parity 0 FAIL（新增
  PDF 必检）；187 测试。
- 机制（最终构建、隔离档案）：T18 8/8（脚本内生成两页 PDF；两页按序、标题为 `#`、
  另存内容与复制一致、拒绝 .txt / 索引外 PDF / 非 PDF；菜单出现两项）；T15 18/18；
  T9 30/30（失焦同样走合成 blur，Windows 拒绝抢前台）；稳定内存 1298 MB。
- 可复现：套件 ×2 各 187 通过逐行一致；T18 ×2 判定逐行一致；T10 14/14、T11 11/11。

## 未验证项

- 界面里点"复制为 Markdown"（会改剪贴板）和"另存为…"（原生对话框）没有自动点；
  两者背后的命令已直测。
- 真实世界 PDF（多栏、复杂表格）的转换质量取决于 pdf-inspector，只用生成的 PDF 测过。
- 扫描页 OCR 走现有引擎，只做了单测注入，未用真实扫描件端到端跑。
...
huodong/Activity Monitor"都搜不到；`mt` 找不到 MacTap；`mac` 不出 MyMacCleaner；
移动到子文件夹的应用还能搜到却打不开，重启后又搜不到。

## 原因与修复

- macOS 只读应用目录第一层 → `find_bundles` 往下看 3 层普通文件夹，不进 `.app`。
- 中文名在包的本地化文件里，只读了文件名 → `bundle_names` 读 `Info.plist`、
  `InfoPlist.loctable`（现代系统应用）、`*.lproj/InfoPlist.strings`（二进制或
  UTF-8/16 文本），只留英文和中文，进别名；界面中文时显示中文名（`localize`，
  切语言会重扫）。Linux `.desktop` 的 `Name[xx]=` 同样处理。
- 首字母只认空格等分隔、且先转小写 → `word_starts` 按原样判断，小写接大写也算
  词首（全大写串不拆）；新增"某个词以查询开头"档 0.75。
- `MyMacCleaner` 其实匹配上了，但上限 4 被四个 mac 开头的应用占满 → 上限 6。
- 列表只在启动扫描 → 唤出时若距上次扫描超过 1 分钟就后台重扫；启动时目标已不在
  就立即重扫并提示（前端中文提示，1.2 秒后重搜让陈旧行消失）。

## 发现并修复

| # | 轮/视角 | 问题 | 修复 |
|---|------|------|------|
| 1 | 编译 | clippy `chunks_exact` | `as_chunks` |
| 2 | 机制（T13 真机） | 首字母 0.5 低于子串 0.6：本机 6 个 "Command Prompt" 含 "mpt"，把 MagpieProbeTool 挤出 | 首字母升到 0.7（词首 0.75 之下、子串之上）+ 单测 |
| 3 | 复查 | 启动失效目标报错是英文、陈旧行不消失 | 前端 i18n 提示 + 延时重搜 |
| 4 | 边界 | `matches_word_start("")` 返回 true（生产有前置判空，函数契约错） | 空查询 false |

## 连续三轮干净（第 4 项之后重新计数）

- 静态一致：clippy 0；a1/a3/docs-parity 0 FAIL；README 声明逐条对到代码（60 s
  节流、30 min、`mt`→MacTap 单测、`huodong` CI 真 Mac）。
- 机制（最终构建、隔离档案）：T13 5/5（上限 6；新应用唤出 421 ms 后用 `mpt` 找到；
  `probe` 词首 0.75；消失的应用启动报"已移动或删除"且列表随即去掉）；T12 5/5。此前
  同代码 T9 30/30、二次启动 141 图标全命中。
- 可复现：套件 ×2 各 173 通过、逐行一致。
- CI 真 Mac（上一次推送）：88 个应用、实用工具 19 个；活动监视器别名含"活动监视器"
  "活動監視器"，`活动`、`huodong` 均能找到；整份列表 489–673 ms（后台线程）。

## 未验证项

- Tikas 机器上的实际效果（中文系统下显示中文名、他那批应用的排序）待其确认。
- Windows 开始菜单名本就是本地化的，未做额外处理。

---

# 验收记录 2026-09-24（0.3.0 在 Mac 上搜应用要好几秒，未出包）

用户在 Mac 上用 0.3.0：应用能搜出来，但要好几秒；问图标能不能缓存。

## 原因

0.3.0 在 macOS 上把读图标放在主线程（启动后逐个预读，搜到应用时按需读），每个
50 ms 到 1 s（CI Mac 实测），几百个应用能占满主线程一两分钟。`search_apps`、
`calc_query` 是同步命令，Tauri 让它们在主线程跑，于是每次按键都排在图标后面；
`resize_palette`（每次渲染都调）和 WKWebView 的键盘事件也走主线程。0.3.0 之前主
线程上没有重活，所以没暴露。图标缓存只在内存里，每次启动都重读全部。

## 修复

- 图标读取在三个平台都改为后台线程。依据：CI 在 macOS 上用后台线程读 11 个图标，
  挂 Apple Main Thread Checker 且 `MTC_CRASH_ON_REPORT=1`，零报告；对照组（后台线程
  创建 NSView）被抓到（"UI API called on a background thread: -[NSView init]"），
  证明检查器生效。后台多线程读 11 个约 600 ms。
- 图标存进数据库 `app_icons`（按启动目标，值为应用修改时间：macOS 取
  `Contents/Info.plist`，其余取快捷方式/.desktop 文件；0 永不命中），下次启动直接
  用，只读新增或更新过的应用，卸载的清掉。
- `search_apps`、`calc_query`、`record_hit_use`、`preview_thumb` 改为
  `#[tauri::command(async)]`，不再进主线程。

## 发现并修复

| # | 轮/视角 | 问题 | 修复 |
|---|------|------|------|
| 1 | A 静态一致 | `read_app_icon` 改为后台线程后 `app` 参数成死参数 | 删除 |

仪器：a1、a3-ipc 的正则只认 `#[tauri::command]`，不认 `(async)`，a3 因此悄悄跳过了
这 4 个命令的参数核对；已改为 `#[tauri::command(...)]` 都认，63/63 定义、80 处调用
复核通过。第一次推 CI 时 clippy 报 doc 列表缩进，脚本没在失败时停下就推了（随即修复）。

## 连续三轮干净

- B 并发/正确（复查 + 测试）：预读与按需读取同一目标只会写相同值；写库失败忽略，
  最坏下次重读；预读不跨 await 持锁。169 测试通过（新增缓存往返/剪枝、stamp、损坏
  行 3 个），clippy 0。
- D 退化输入：库里未知类型/缺图像/缺类型的行都读成"无图标"而不报错，stamp 仍生效；
  stamp 取不到为 0 永不命中；卸载的应用被剪枝。
- C 可复现：套件 ×2 各 169 通过、排序后逐行一致；隔离档案两次启动测试跑两遍，结果
  一致：首启"0 命中、读 141、7.2–7.4 s"，再启"141 命中、读 0、10–13 ms"；T9 均 30/30。

## 追加：后台定时刷新（用户提议"缓存是否需要定期更新，后台一个线程就行"）

原先应用列表只在启动时扫一次：常驻期间新装的应用搜不到、更新过的图标不刷新。现
在独立定时器每 30 分钟（不跟文件重扫的设置走，那个可以关）重扫列表 + 按修改时间
对比缓存，只读新增/更新的，删除卸载的；有变化时发 `app-icons-changed`，前端清掉
自己那份图标并让屏上的行重取。全在后台线程。测试钩子 `MAGPIE_APP_RESCAN_SECS`。
安静的刷新只记 debug，首轮和有变化的轮记 info。

| # | 轮/视角 | 问题 | 修复 |
|---|------|------|------|
| 2 | 机制（T12 多等 35 秒） | 第一次安静刷新仍记了 info：`changed \|\| !swap()` 短路，有变化的轮从不置位 | 先 swap 再判断 |

连续三轮干净（第 2 项之后重新计数）：
- 机制通路 T12（隔离档案，15 秒刷新；脚本自建/自删开始菜单探针快捷方式）5/5：
  新应用 2.6 s 可搜、带图标、前端收到 1 次事件；删除后 15 s 内消失；之后 35 s 的
  安静刷新无 info 行。
- 可复现：T12 再跑一遍，判定与日志序列相同（0/141 读 → 141/1 读 → 1 删，安静轮
  6–7 ms）。
- 静态/复查：170 测试；a1/a3/docs-parity 0 FAIL；事件 1 处发、1 处听；README 双语
  补"每 30 分钟后台刷新"。

## 未验证项

- Mac 上的实际打字延迟：本机无 Mac，结构上已去掉主线程上的图标工作，需用户在新
  版本上确认。
- 快捷方式不变而程序自身换了图标（Windows）时，缓存的旧图标会一直用到快捷方式变化。
...
的全量遍历（按 mtime/size 跳过未变文件）。用户采纳建议：加 `notify` 监听 +
去抖 + 范围重扫，30 分钟全量保留兜底并做成可调，加"立即重扫"。

## 实现

- `core/files.rs`：扫描重构为 `Scan`（规则、逐文件 upsert、known/seen、按范围
  剪枝），全量 `index_folders` 与新增 `index_changed` 共用同一套规则。
  `index_changed` 把事件路径映射为：文件 → 重读其所在目录（深度 1）；目录 →
  整棵子树；不存在 → 删掉该路径及其下所有行并重读父目录（重命名 = 旧名消失 +
  新名出现）。根目录之外、点开头组件之下、被剪枝的 worktree 内的路径一律忽略；
  剪枝只发生在本次触及的范围内，索引其余部分与上次全量遍历一致。
- `src-tauri/watch.rs`（纯逻辑）：`Pending` 合并事件，磁盘静默 2 秒后成批，
  超过 2000 条或 OS 报溢出转为一次全量；索引器忙则放回队列。`is_noise` 在入口
  丢弃根目录以下点开头组件的事件（`.git` 为主）。
- `lib.rs`：`notify` 8.2 `RecommendedWatcher` 递归监听每个已注册文件夹，启动、
  增删文件夹、开关时重挂；1 秒 ticker 把批次交给 `start_local_index(Scope)`；
  周期循环改为每分钟读 `rescan_minutes`（默认 30，0 关），改设置无需重启；
  命令 `set_watch` / `set_rescan_minutes`，已有的 `index_local` 接到界面。
- 设置两行："文件变化"开关 + 监听文件夹数，"全量重扫" 5/15/30/60 分钟/关 +
  "立即重扫"。README 双语、落地页一条、导出键补两项。

## 发现并修复

| # | 视角 | 问题 | 修复 |
|---|------|------|------|
| 1 | 逻辑/退化 | `.git` 目录下成千上万的对象文件事件会灌满 pending，超 2000 条就触发全量遍历，而它们本就不进索引 | 入口 `is_noise` 过滤根目录以下点开头组件；根自身含点不算 |
| 2 | 文案 | 中文"正在监听 2 个文件夹"后跟英文句号 | 去掉 JSX 硬编码句号 |

仪器：vite 只绑了 `::1`，Chrome 走 127.0.0.1 超时，改 `--host 127.0.0.1`；发布
构建占满 CPU 时页面首载超 30 秒，脚本超时放宽到 120 秒；T7 的告警正则误匹配
INFO 行，改为只匹配 WARN。

## 连续四轮干净（第 2 项之后重新计数；第 1 项之后 Rust 侧再跑一遍）

- A 静态一致：a1 10/10、a3-ipc 3/3、docs-parity 26/26（必检项加 watcher）、
  round1 24/24
- B 代码正确：clippy 全 target 0；160 测试（新增 13：`index_changed` 集成 7、
  `Pending`/`is_noise` 单元 6）
- C 机制通路：无头 T6 8/8（默认开 + 计数、关/开、间隔 15/关/30、立即重扫可点）；
  真机 T7（生产 exe）10/10：监听 2/2 文件夹 0.8 秒就绪；新建 2.9 秒、追加 3.0
  秒、新目录内文件 3.5 秒、目录重命名 2.7 秒、删除 2.9 秒进索引；未动的文件保留；
  日志 5 次范围重扫、0 告警；清理 0 残留
- E 可复现：套件 ×3 均 160/0

## 未验证项

- macOS（FSEvents）/ Linux（inotify，含 watch 数上限）未跑真机；`notify` 在三
  平台的路径形式与 walker 一致性只在 Windows 验证过。
- 周期循环改为按分钟读设置，逻辑简单未做端到端等待验证（需 ≥2 分钟）。
- 同一秒内改动且大小不变仍会漏（mtime 秒级），与全量遍历一致。
...
新探针；两个真 bug 都是新探针抓到的。

## 新增的探针

- `a3-ipc.js`（静态）：前端每个 `invoke()` 的参数键与 Rust 命令签名逐一比对
  （Tauri 把 camelCase 映射成 snake_case，对不上会静默传 undefined）；TS
  `Status` 接口的每个字段必须在 `get_status` 的 JSON 里出现；i18n 无死键
- `core/tests/invariants.rs`（不变量 + 边界）：过滤解析对自身剩余文本幂等、
  `slug`/`lines` 幂等、日期加减往返、笔记超长/怪字符、worktree 指针非 UTF-8；
  两个确定性模糊循环（xorshift 固定种子，字母表含 CJK / 组合符 / 零宽 / emoji /
  各种前缀），4 万 + 1.5 万条随机输入打 `calc::eval` / `transform` /
  `filters::parse` / `matches`，要求永不 panic
- `a_burst_of_searches_settles_with_the_newest_running_last`（并发）：100 个
  并发搜索票，10 秒内全部落定、无死锁、连接仍健康

## 发现并修复

| # | 视角 | 问题 | 修复 |
|---|------|------|------|
| 1 | 模糊输入 | `transform::color` 按**字节**长度分支后 `&hex[0..2]` 切片，`#报销` 是 6 字节 → 切在字符中间 panic。输入框里输 `#报销` 就会炸掉 `calc_query` | 先要求全部字节是 ASCII 十六进制再切；加用例 `#报销` / `#报` / `#ffz600` / `#ff660` |
| 2 | 边界输入 | `Filters::matches` 里 `now - days * 86_400`，`99999999999999999d` 这类超大天数整数溢出：debug panic、release 静默回绕导致匹配错误 | `saturating_mul` / `saturating_sub`；加极值用例 |
| 3 | clippy | 新测试文件 3 条提示（`trim()` 接 `split_whitespace()`、可折叠 if） | 抽出 `bare_verb` 助手 |
| 4 | 我的测试 | 极值用例期望写反（时钟 MAX + 饱和跨度 → 截止点恰为 0，负 mtime 应被排除） | 改期望并注明语义 |

第 4 项修的是测试而非产品，但按规则同样清零重计。

## 连续五轮干净（第 4 项之后重新计数）

- A 静态一致：命令面 55 定义 = 55 注册全引用；i18n 140+28 键全有中文、232 键
  无死键；22 个 meta 设置项写读平衡；69 个 invoke 调用点参数全匹配；38 个
  Status 字段全有来源；README/站点双语对称 0 FAIL
- B 代码正确性：clippy 全 target 0；126 测试连跑 3 次逐次一致（含新增 10 个）；
  tsc + vite 通过
- C 机制通路（无头 demo）：特性总回归 19/19、最近打开 5/5、删完文件夹 6/6
- D 边界与退化：5.5 万条随机输入零 panic；过滤器 9/9 含极值；日期往返
  1/7/30/365/1000 天全部回到原点
- E 真机（生产 exe）：worktree 剪枝 → 关 → 开 4/4，清理 0 残留

## 仪器坑（记账）

- 真机脚本清理步骤上次只手工修了库、没改脚本，这次又留了 1 行；脚本已改为
  `PRAGMA foreign_keys=ON` + 显式删 files 行，本次 0 残留
- 无头统计把 "FAIL count: 0" 这行当成一条 FAIL，改为只数 `^FAIL |`
- 套件 ×3 的通过数统计漏掉 `test result: FAILED. N passed` 行，改为两种都算

## 教训

模糊测试半小时抓到两个 panic，而此前三十多轮"人工 review"没看见——按字节切
UTF-8 字符串、无饱和的整数算术，这两类在纯函数里最该由机器找。`invariants.rs`
以后随套件常跑（fuzz 循环 0.2 秒）。
