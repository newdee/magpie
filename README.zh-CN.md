...
- **尊重机密的剪贴板历史**：默认关、本地存储，密码管理器标记为机密的内容永不
  ...
  `Shift+Enter` 直接粘贴回你刚才所在的应用。
- **复制的图片也进历史**：截图和复制的图片带缩略图入库，之后**用描述找回**
  （"那个报错弹窗"）——SigLIP 对剪贴板同样生效。
- **截图里的字能搜，视频里的字也能搜**：可选 OCR（PP-OCRv4，约 15 MB，
  中英文）识别已索引图片和每个视频镜头帧里的文字——字幕、PPT、屏幕上的
  代码都变成可搜的，视频命中直接跳到文字出现的时刻。设置里可选
  PP-OCRv4（15 MB）或更准的 PP-OCRv6 small（30 MB），默认关闭。独立子开关还能识别**扫描版 PDF**（无文字层的页面）——大部头
  较耗时，开不开由你。
- **越用越懂你**：真正被打开的结果随时间上浮（频次 + 近期加权衰减）——
  纯本地统计，永远压不过更优匹配。
...
- **输入框自带计算器**：`3*(5+2)^2`、`0xff + 1`、`100 mb to gb`、
  `32 f to c`——结果显示在首行，`Enter` 复制。
- **网页快搜**：`gh magpie` 直达 GitHub 搜索、`g …` Google、`bd …` 百度——
  前缀规则可在设置里编辑（`前缀 = 带 {q} 的 URL`）。
- **表情查找**：输入 `:` 加关键词（`:fire`、`:火`），点击或 `Enter` 复制。
- **文件命中带后续动作**：`Ctrl+C` 复制路径，`Ctrl+Shift+C` 把文件本体放上
  剪贴板——粘到微信/邮件直接变附件。
- **自动保持最新**：从这版起签名校验的原地自动更新——无需重装。启动时
...
（`3:24 – 3:42`）。本地 tab 的 **videos** 范围把文件名匹配和画面描述搜索
合在一个列表里。解码用 ffmpeg——自动使用系统安装，缺失时从 magpie 自己的
Release 一次性下载静态版（能下 magpie 就能下它；GPL 构建，源码见
ffmpeg.org）。设置里可开关，并显示 ffmpeg 状态。
...
搜索开关 · 解码限制（线程/硬解） · 标签页顺序与默认标签页 · 剪贴板历史
