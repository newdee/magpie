...
- **全屏应用之上也能唤出**（macOS）：浮窗直接进入你当前所在的 Space，全屏的
  浏览器、终端也一样。macOS 上 magpie 住在菜单栏，不占 Dock、不进 Cmd-Tab，
  和 Spotlight 一样。
...
- **时区换算，离线可用**：`东京时间`、`time in new york`、`tokyo time`，
  或 `3pm pst to beijing`、`9am to london`（从本地时间换算）。夏令时按日期
  自动处理；单独输 `tokyo` 仍然是普通搜索。
- **系统命令**：锁屏、睡眠、重启、关机、清空废纸篓（回收站）、切换深色模式，
  中英文名都认（`lock`、`锁屏`、`sp`）。重启、关机和清空废纸篓要按两次 `Enter`。
- **结束进程**：`kill chrome` 按名字列出运行中的进程和占用内存，按两次 `Enter`
  结束选中的那个。`port 3000`（或 `kill :3000`）则列出正在监听这个端口的进程。
  magpie 自己和系统进程永远不会列出。
- **截图取字**：单独输入 `ocr`，读出剪贴板里那张图片上的文字（比如刚截的图），
  `Enter` 复制。用的是 OCR 引擎，所以要先在设置里打开「图片文字（OCR）」。
- **本机 IP**：`ip` 显示本机在局域网里的地址，真实网卡排在虚拟网卡（WSL、
  Docker）前面。直接从网卡读取，离线可用，不往外发任何请求。
- **最近下载**：`dl`（或 `最近下载`）列出「下载」文件夹里最新的文件，不用建索引；
  `Enter`、`Ctrl+K` 这些操作和普通文件一样。还没下完的文件不会列出来。
- **金额大写和拼音**：`大写 1234.56` 得到「壹仟贰佰叁拾肆元伍角陆分」，报销、写合同
  都用得上。`py 重庆` 得到 *chóng qìng*，带声调；银行、长大这类常见多音词能读对，
  其他多音字按最常用的读音。
- **全半角和转义**：`半角`、`全角` 转换字母、数字和符号（中文句子里的标点保持全角）；
  `unicode`、`html` 负责转义和反转义。不带内容时都处理剪贴板。
- **二维码双向**：剪贴板里是图片时，`qr` 读出图中的二维码；是文字时，就把文字画成
  二维码。
- **对比两次复制**：`diff` 逐行显示最近两次复制的文字有什么不同，需要打开剪贴板历史。
- **倒计时**：`timer 25m 开会` 开始倒计时，结束时弹出系统通知。单独输入 `timer`
  列出正在进行的倒计时，`Enter` 全部停止。
- **系统状况和随机**：`sys` 显示 CPU、内存和各磁盘剩余空间。`random 1 100`、
  `pick 火锅 烧烤 麻辣烫`、`dice`（骰子）、`coin`（抛硬币）。
- **每一行都有操作菜单**：`Ctrl+K`（macOS 上 `⌘K`）列出选中结果能做的事：用默认
  应用打开或在文件夹中显示、复制路径或文件本体、在终端中打开文件所在的文件夹
  或用已安装的编辑器（VS Code、Cursor、Trae、Zed、Sublime Text 等）打开文件、把文件
  移到回收站或废纸篓（按两次 `Enter` 确认）、以管理员身份运行应用（Windows）、
  复制仓库的 clone 命令或把网页复制为 Markdown 链接、把剪贴板历史里的图片另存为
  文件、钉住或删除剪贴条、结束进程。
...
- **表情和符号查找**：输入 `:` 加关键词（`:fire`、`:火`），点击或 `Enter` 复制。
  不是 emoji 的常用符号也在里面，比如 `:arrow` →、`:乘` ×、`:rmb` ¥、`:cmd` ⌘；
  `:sym` 列出全部符号。
...
- 图片模型只在用得上时才加载：索引文件夹里有图片或视频，或剪贴板历史里有
  图片。只索引文字的话，它一点内存都不占。
...
状态。预览面板里视频显示它的镜头；镜头索引还没轮到它时显示一帧画面（需本机
已有 ffmpeg，预览不会触发下载）。没有可渲染内容的文件改为显示所在文件夹、
大小和修改时间。
...
URL、文件夹路径检索；输入的词一个都没匹配上时，才按意思给出最多三条，并标注「可能相关」。同一个页面在多个浏览器里都有时只占一行，后面列出各浏览器的图标。`Enter` 在默认浏览器打开。
...
| `↑` `↓` / `PgUp` `PgDn` | 选择 / 翻页 |
| `Enter` | 按类型动作：repo/书签/历史进浏览器、文件在资源管理器定位、启动应用、复制剪贴条。做完后输入框清空，下次唤出是空的（`Esc` 收起则保留） |
| `Ctrl+Enter` | 把输入交给浏览器：像网址就直接打开，否则网页搜索 |
| `Ctrl+C` | 复制这一行的标识：路径、网址或剪贴条文本（`Ctrl+Shift+C`：文件本体） |
| `Ctrl+K` | 选中结果的操作菜单（`↑↓` 选择，`Enter` 执行，`Esc` 关闭） |
| `Ctrl+Alt+Space` | 在任何应用里：搜索选中的文字（macOS 是 `Option+Shift+Space`；可改键） |
| `Tab` | 下一个数据源（本地 / Stars / Web / 剪贴板，顺序在设置里定） |
| `Ctrl+1` … `Ctrl+9` | 直接切到对应序号的 tab，按 tab 栏顺序（设置里可改成 `Alt` 或关闭） |
| `Shift+Tab` | 循环当前源的模式：本地范围（全部/文本/图片）、Web 范围（全部/书签/历史）、star 排序 |
| `Shift+Enter` | 把选中剪贴条直接粘贴到上一个应用（剪贴板 tab） |
| `Shift`+`↑` `↓` | 扩展多选（剪贴板 tab） |
| `Ctrl+Delete` | 删除选中的剪贴条（剪贴板 tab） |
| `→` / `←` | 展开 / 收起预览面板（`→` 需光标在输入末尾） |
| `Alt+,` 或 `Ctrl+,`（macOS 上 `⌘,`）| 设置 ↔ 搜索 切换 |
| `Esc` | 关闭操作菜单 → 清除图片查询 → 关闭设置 → 收起窗口 |
| 拖拽 / 粘贴 / 选图 | 以内容搜本地图片 |

macOS 上表里写 `Ctrl` 的地方用 `Cmd` 同样有效。

浮窗固定在屏幕中上、置顶于所有窗口，按住顶部标签条可拖动。点到别的窗口时
浮窗收起：macOS 默认开启；Windows 和 Linux 默认保持显示直到手动收起，方便从
...
浅色/深色）· 界面语言（跟随系统 / English / 中文）· 开机启动 · 失焦时隐藏 ·
数字键切换 tab（Ctrl/Cmd、Alt 或关闭）· 拼音匹配应用开关 ·
